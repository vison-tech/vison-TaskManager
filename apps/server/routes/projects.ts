...
export function registerProjectRoutes(app: Hono, storage: Storage): void {
  ...
  app.delete('/api/v1/projects/:id', (c) => { const id = routeId(c.req.param('id')); storage.write(() => storage.deleteProject(id, versionQuery(c.req.query('version')))); return c.body(null, 204); });
}
